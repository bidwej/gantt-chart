...
Gantt.utils = {
  ...
  createDateParser(format) {
    ...
    format.replace(/(yyyy|yy|y|MMMM|MM|M|dd|d|HH|H|mm|m|ss|s|SSS|SS|S)/g, (part) => {
  ...
  hasClass(el, className) {
    ...
    if (el.className && el.className.match) {
      ...
      const escapedClassName = this.escapeRegExp(className);
      return !!el.className.match(new RegExp(`(\\s|^)${escapedClassName}(\\s|$)`));
    }
    return false;
  },
  ...
  removeClass(el, className) {
    if (el.classList) {
      ...
      const escapedClassName = this.escapeRegExp(className);
      const reg = new RegExp(`(\\s|^)${escapedClassName}(\\s|$)`);
  ...
  walkToAncestor(parent, elt, cb, param) {
    function makeArrayMatcher(array) {
      return function arrayMatcher(obj) {
  ...
  propertyEvaluator(path) {
    ...
    const ar = path.split('.');
    if (ar.length > 1) {
      return function nestedPropertyEvaluator(obj) {
    ...
    if (ar.length === 1) {
      return function directPropertyEvaluator(obj) {
        return obj[path];
      };
    }
    return function identityEvaluator(obj) {
      return obj;
    };
  },
  ...
  implements(clazz) {
    for (let i = 1, count = arguments.length; i < count; i++) {
      ...
      for (const key in toImplement) {
        if (Object.prototype.hasOwnProperty.call(toImplement, key)) {
...
Gantt.utils.appendSVG = function appendSVG(elt, cfg) {
...
/*                                                                                      */
/*                                                                                      */
var dateFormat = (function dateFormatIIFE() {
  ...
  return function formatDate(date, mask, utc) {
    const dF = dateFormat;
...
// Standalone formatDate utility (avoids Date.prototype pollution)
Gantt.utils.formatDate = function formatDateStandalone(date, mask, utc) {
  return dateFormat(date, mask || Gantt.utils.defaultDateFormat, utc);
};

Gantt.defaultPaletteName = 'qualitative20';
Gantt.defaultPalettes[Gantt.defaultPaletteName] = new (getComponent('Palette', Gantt.components.Palette))([
